...
namespace dns {
    ...
    void UdpServer::send_response(std::size_t bytes_received) {
        ...
        response_buffer = handle_query_rr(request);
    ...
    Buffer handle_query_rr(const Buffer& request) {
        ...
        std::uint16_t transaction_id = query.header.id;
        
        bool rd = query.header.flags.rd;
        if(rd) {
            // recursive resolution
            std::cout << "I'm doin rec resolution!\n";

            RecursiveResolver rr(query);
